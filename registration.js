 ...
 const loader = document.getElementById('loader');
  ...
  if (registrationUsername.trim() == "" || registrationEmail == "" || registrationPhone == "" || registrationPassword == "" || registrationConfirmpassword == "") {
    ...
    if (!usernameRegex.test(registrationUsername)) {
            ...
            document.getElementById("registration-username-error").textContent = "Spaces and special characters not allowed. Only numbers and letters";
            return;
        } else {
...
function mpesePayment(){
const mpesapaymentForm = document.getElementById("mpesapaymentmethod_form");

mpesapaymentForm.addEventListener("submit", async (event) => {
    event.preventDefault();
    const loader = document.getElementById('loader');
    const paymentmethodUsername = document.getElementById("name").value;
    const paymentmethodPhone = document.getElementById("phone_number").value;
    const phoneRegex = /^(07|01)\d{8}$/;
    const nameRegex = /^[a-zA-Z]+\s+[a-zA-Z]+$/;
    // Validate email and password
    if (paymentmethodUsername.trim() == "" || paymentmethodPhone == "") {
      document.getElementById("paymentmethod-emptyfield-error").style.color = "yellow";
      document.getElementById("paymentmethod-emptyfield-error").textContent = "Please fill all the fields";
      return;
    } else {
      document.getElementById("paymentmethod-emptyfield-error").textContent = "";
    }
    if (!nameRegex.test(paymentmethodUsername)) {
      document.getElementById("paymentmethod-username-error").style.color = "yellow";
      document.getElementById("paymentmethod-username-error").textContent = "Provide a valid legal Name.";
      return;
    } else {
      document.getElementById("paymentmethod-username-error").textContent = "";
    }
    if (!phoneRegex.test(paymentmethodPhone)) {
      document.getElementById("paymentmethod-phone-error").style.color = "yellow";
      document.getElementById("paymentmethod-phone-error").textContent = "Please provide a valid phone number.";
      return;
    } else {
      document.getElementById("paymentmethod-phone-error").textContent = "";
    }

    // If validation passes, you can submit the form
    document.getElementById("mpesapaymentmethod_form").submit();
...
const paypalpaymentForm = document.getElementById("paypalpaymentmethod_form");

paypalpaymentForm.addEventListener("submit", async (event) => {
    event.preventDefault();
    const loader = document.getElementById('loader');
  const paypalpaymentmethoEmail = document.getElementById("email").value;
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  // Validate email and password
  if (paypalpaymentmethoEmail.trim() == "") {
    document.getElementById("paypalpaymentmethod-emptyfield-error").style.color = "red";
    document.getElementById("paypalpaymentmethod-emptyfield-error").textContent = "Please fill your email address";
    return;
  } else {
    document.getElementById("paypalpaymentmethod-emptyfield-error").textContent = "";
  }
  if (!emailRegex.test(paypalpaymentmethoEmail)) {
    document.getElementById("paypal-email-error").style.color = "red";
    document.getElementById("paypal-email-error").textContent = "Please provide a valid email address.";
    return;
} else {
    document.getElementById("paypal-email-error").textContent = "";
} 
alert("Payment added successfully");
  // If validation passes, you can submit the form
  document.getElementById("paypalpaymentmethod_form").submit();
  loader.style.display = 'block';
